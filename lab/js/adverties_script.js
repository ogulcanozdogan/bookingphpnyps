...
$('#txtDate').datepicker({
	...
	$('#txtDate2').datepicker({
		...
		showOn: 'button',
		onClose: function(dateText, inst) {
			$('#year2').val(dateText.split('/')[2]);
			$('#month2').val(dateText.split('/')[0]);
			$('#day2').val(dateText.split('/')[1]);
		}
	});
	$('#txtDate3').datepicker({
		buttonImage: 'images/calendar.png',
		...
		showOn: 'button',
		onClose: function(dateText, inst) {
			$('#year3').val(dateText.split('/')[2]);
			$('#month3').val(dateText.split('/')[0]);
			$('#day3').val(dateText.split('/')[1]);
		}
	});
...
$("#success").text("");
    ...
    $("#loading").text("Submitting...");
		var form = $('#actionForm')
        var formAction = 'pedicab_advertising.php';
	...
	$("#actionForm2").submit(function (event) {
		$("#success2").text("");
		$(".submit-btn2").attr("disabled",true);
		$("#loading2").text("Submitting...");
		var form = $('#actionForm2')
		var formAction = 'pedicab_advertising_without.php';
		var serializedFormData = form.serialize();
		$.ajax({
			type: "POST",
			url: formAction,
			...
			if(data.success==true){
				$("#success2").text("Successfully Sent!!!");
				document.getElementById("actionForm2").reset();
				$(".submit-btn").attr("disabled",false);
				$("#loading2").text("");
				$("#FirstName2").empty();
				$("#Email2").empty();
