   ...
   <body>
      ...
      <script>
         $("#phoneNumber").intlTelInput({
             initialCountry: "us", // Automatically select user's country
             separateDialCode: true, // Show phone code as a separate field
             utilsScript: "https://cdnjs.cloudflare.com/ajax/libs/intl-tel-input/17.0.8/js/utils.js" // Helper script for formatting
         });
      </script>
      <script>
         ...
         phoneNumberInput.addEventListener("input", function(event) {
             ...
             // Check for 10-digit phone number format
             var phoneNumberRegex = /^(\d{3})(\d{3})(\d{4})$/;
             if (phoneNumberRegex.test(phoneNumber)) {
                 // Format the number with parentheses and hyphen
                 var formattedPhoneNumber = phoneNumber.replace(phoneNumberRegex, "($1) $2-$3");
         
                 // Set the formatted number in the input field
