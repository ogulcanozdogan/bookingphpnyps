...
$satir = [
    ...
    'number' => $number
			   ...
			   $sql = "INSERT INTO users (user, pass, name, surname, email, number)
VALUES ('$user', '$pass', '$name', '$surname', '$email', '$number')";
			$durum = $baglanti->prepare($sql)->execute();
               

             if ($durum)
             {
                echo '<script>swal("Successful","Registration successful.","success").then((value)=>{ window.location.href = "login.php"});
        ...
        echo '<script>swal("Hata","' . $hata . '","error");</script>';
	 			...
	 			?>
              </div>  <a href="login.php" class="btn btn-primary" > Sign In</a>
            </div>
          </div>
        </div>
      </div>
...
form.addEventListener('submit', function (e) {
    // Telefon numarasının formatını kaldır, ancak formu normal şekilde gönder
    const rawNumber = phoneNumberInput.value.replace(/\D/g, ''); // Formattan kurtul
    phoneNumberInput.value = rawNumber; // İnput değerini güncelle
    // Formu gönder, bu yüzden preventDefault() çağrısını kaldır
});

// Form gönderildikten sonra PHP tarafında form verileriyle işlem yapılabilir.
